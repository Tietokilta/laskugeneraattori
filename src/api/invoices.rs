...
fn finnish_reference_check_digit(base: &str) -> Option<u8> {
...
fn is_valid_finnish_reference_number(value: &str) -> bool {
    ...
    let expected = match finnish_reference_check_digit(base) {
        Some(digit) => digit,
        None => return false,
    };
...
pub struct Invoice {
    ...
    /// Finnish reference number generated by the server
    #[garde(skip)]
...
pub async fn create(
    ...
    multipart.data.attachments = attachments
        ...
        .collect();
    multipart.data.reference_number = generate_finnish_reference_number();
    assert!(
        is_valid_finnish_reference_number(&multipart.data.reference_number),
        "BUG: generated invalid Finnish reference number"
    );
