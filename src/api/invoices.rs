...
// NOTE: A missing cost pool is allowed and falls back to the unassigned account, but an id we
// don't recognise is a client bug and is rejected loudly
fn is_known_cost_pool(value: &Option<String>, _: &()) -> garde::Result {
    match value {
        None => Ok(()),
        Some(id) if cost_pools::get(id).is_some() => Ok(()),
        Some(id) => Err(garde::Error::new(format!("unknown cost pool: {id}"))),
    }
}
...
pub struct Invoice {
    ...
    /// The id of the toimikunta whose account the payment is booked against, must be one of the
    /// ids returned by `GET /cost-pools`. If omitted, the invoice is booked against a
    /// non-existent account so that the treasurer notices it needs assigning by hand.
    #[garde(custom(is_known_cost_pool))]
    #[serde(default)]
...
pub async fn create(
    ...
    let cost_pool = cost_pools::resolve(multipart.data.cost_pool.as_deref());
