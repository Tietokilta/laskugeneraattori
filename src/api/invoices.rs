...
fn is_valid_cost_pool_id(value: &Option<String>, _: &()) -> garde::Result {
    match value {
        None => Ok(()),
        Some(id) if id.len() == 24 && id.chars().all(|c| c.is_ascii_hexdigit()) => Ok(()),
        Some(id) => Err(garde::Error::new(format!("not a cost pool id: {id}"))),
    }
}
...
pub struct Invoice {
    ...
    /// The id of the cost pool (toimikunta) in the CMS whose account the payment is booked
    /// against. If omitted, unknown to the CMS, or unresolvable because the CMS is unreachable,
    /// the invoice is booked against a non-existent account so that the treasurer notices it
    /// needs assigning by hand.
    #[garde(custom(is_valid_cost_pool_id))]
    #[serde(default)]
    pub cost_pool: Option<String>,
    // NOTE: Resolved from the CMS by the server, any client-supplied value is ignored
    /// The name of the toimikunta the invoice is booked against
    #[garde(skip)]
    #[serde(default, skip_deserializing)]
    #[schema(read_only)]
    pub cost_pool_name: String,
    // NOTE: The reference number is generated by the server, any client-supplied value is ignored
    /// The Finnish reference number of the invoice
    #[garde(skip)]
    #[serde(default, skip_deserializing)]
    #[schema(read_only)]
...
pub async fn create(
    ...
    axum::extract::State(cost_pools): axum::extract::State<CostPoolClient>,
